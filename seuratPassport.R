...
seuratPassport  <- function(seurat_obj, parent = NULL, read = FALSE) {
  ...
  # Safety check
  if (is.null(seurat_obj)) stop("seurat_obj is NULL — please reload your object first!")
  ...
  server <- function(input, output, session) {
    shiny::observeEvent(input$done, {
      ...
      for (i in seq_along(custom_keys)) {
        key <- trimws(input[[paste0("custom_key", i)]])
        val <- trimws(input[[paste0("custom_val", i)]])
        if (nchar(key) > 0 && nchar(val) > 0) custom[[key]] <- val
      }
